...
file_location = "/user/jliu2/fare.csv"
file_type = "csv"
...
file_location = "/user/jliu2/trip.csv"
file_type = "csv"
...
temp_table_name1 = "fare_csv"

df1.createOrReplaceTempView(temp_table_name1)

# COMMAND ----------

temp_table_name2 = "trip_csv"
...
if IS_SPARK_SUBMIT_CLI:
    fareSample = spark.read.csv('/user/jliu2/fare.csv', inferSchema=True, header=True)
else:
    fareSample = sqlContext.sql("select * from fare_csv")

# COMMAND ----------

if IS_SPARK_SUBMIT_CLI:
    tripSample = spark.read.csv('/user/jliu2/trip.csv', inferSchema=True, header=True)
else:
    tripSample = sqlContext.sql("select * from trip_csv")
...
databe = data.select('passenger_count', 'trip_time_in_secs', 'trip_distance', col(' fare_amount').alias('label'))
data1=databe.dropna(how = 'any')
# COMMAND ----------
